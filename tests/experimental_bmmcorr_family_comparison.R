...
fit_bmm <- function(formula, data, tree, structure, bmm.scale = TRUE) {
  args <- list(
    formula,
    data = data,
    tree = tree,
...
compare_case <- function(case_type, relation, n_tips, signal, replicate, seed) {
  ...
  fit_coronly <- fit_bmm(Y ~ 1, data = list(Y = Y), tree = tree, structure = "corrpower", bmm.scale = FALSE)
  fit_corrpower <- fit_bmm(Y ~ 1, data = list(Y = Y), tree = tree, structure = "corrpower", bmm.scale = TRUE)
  ...
  data.frame(
    ...
    corrpower_noscale_cov_err = cov_error(fit_coronly$sigma$regime[names(true_sigma)], true_sigma),
    ...
    corrpower_noscale_logLik = as.numeric(fit_coronly$logLik),
    corrpower_logLik = as.numeric(fit_corrpower$logLik),
    corrpower_noscale_boundary = isTRUE(diag_flag(fit_coronly, "boundary_corr_power", FALSE)),
    corrpower_boundary = isTRUE(diag_flag(fit_corrpower, "boundary_corr_power", FALSE)),
    corrpower_noscale_pathological = isTRUE(diag_flag(fit_coronly, "pathological_corr_power", FALSE)),
...
assert_true(
  stats::median(coronly_cases$corrpower_noscale_cov_err) <= stats::median(coronly_cases$corrpower_cov_err) + 0.05,
  "corrpower with bmm.scale=FALSE should match or beat full corrpower in correlation-only scenarios"
)
assert_true(
  mean(coronly_cases$corrpower_noscale_boundary) <= mean(coronly_cases$corrpower_boundary) + 0.10,
  "corrpower with bmm.scale=FALSE should not have materially worse boundary behavior in correlation-only scenarios"
)
assert_true(
  mean(coronly_cases$corrpower_noscale_pathological) <= mean(coronly_cases$corrpower_pathological) + 0.05,
  "corrpower with bmm.scale=FALSE should not have materially worse pathology in correlation-only scenarios"
)
assert_true(
  stats::median(full_cases$corrpower_cov_err) <= stats::median(full_cases$corrpower_noscale_cov_err) + 0.05,
