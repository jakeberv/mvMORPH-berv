...
bootstrap_runtime <- function(repo_root) {
  ...
  install_lib <- Sys.getenv("MVSIMMAP_OUM_EB_BENCH_R_LIB", unset = "")
  ...
  if (length(missing_pkgs)) {
    ...
    utils::install.packages(
      missing_pkgs,
      repos = Sys.getenv("MVSIMMAP_OUM_EB_BENCH_CRAN_REPO", unset = "https://cloud.r-project.org"),
      lib = install_lib,
      quiet = TRUE
    )
  }
...
choose_regime_nodes <- function(tree,
  ...
  if (!length(B_candidates)) return(NULL)
  ...
  if (!length(C_candidates)) return(NULL)
...
make_true_param <- function() {
  ...
  alpha_C <- diag(2.5, 3)
  dimnames(alpha_C) <- list(trait_names, trait_names)
  beta_B <- -2.0

  list(
    theta = theta,
    sigma = list(A = sigma_A, B = sigma_B, C = sigma_C),
    alpha = list(C = alpha_C),
    beta = c(B = beta_B)
  )
}
...
fit_mixed_model <- function(tree, data, process, method, decomp) {
  mvSIMMAP(
    tree, data,
    process = process,
    method = method,
...
flatten_true_fit <- function(fit, rep_id, seed) {
  ...
  for (i in seq_len(nrow(alpha))) {
    out[[sprintf("alpha_C_%d%d", i, i)]] <- alpha[i, i]
  }

  out$beta_B <- fit$beta[["B"]]
  out$AIC_true <- fit$AIC
  out$AICc_true <- fit$AICc
  out$logLik_true <- as.numeric(logLik(fit))
  out$nparam_true <- fit$param$nparam
  out$true_convergence <- fit$convergence
  out$true_hessian <- fit$diagnostics$hessian$status
  out
}
...
truth_vector <- function(true_param) {
  c(
    ...
    sigma_B_11 = true_param$sigma$B[1, 1],
    sigma_B_12 = true_param$sigma$B[1, 2],
    sigma_B_13 = true_param$sigma$B[1, 3],
    sigma_B_22 = true_param$sigma$B[2, 2],
    sigma_B_23 = true_param$sigma$B[2, 3],
    sigma_B_33 = true_param$sigma$B[3, 3],
    sigma_C_11 = true_param$sigma$C[1, 1],
    sigma_C_12 = true_param$sigma$C[1, 2],
    sigma_C_13 = true_param$sigma$C[1, 3],
    sigma_C_22 = true_param$sigma$C[2, 2],
    sigma_C_23 = true_param$sigma$C[2, 3],
    sigma_C_33 = true_param$sigma$C[3, 3],
    alpha_C_11 = true_param$alpha$C[1, 1],
    alpha_C_22 = true_param$alpha$C[2, 2],
    alpha_C_33 = true_param$alpha$C[3, 3],
    beta_B = true_param$beta[["B"]]
  )
}
...
summarize_model_comparison <- function(results) {
  model_cols <- c("AIC_true", "AIC_no_EB", "AIC_no_OUM", "AIC_BMM", "AIC_BM1")
  ...
  colnames(weights) <- c("weight_true", "weight_no_EB", "weight_no_OUM", "weight_BMM", "weight_BM1")
  ...
  data.frame(
    metric = c(
      ...
      "mean_AIC_BMM",
      "mean_AIC_BM1",
      "mean_delta_AIC_no_EB_minus_true",
      "mean_delta_AIC_no_OUM_minus_true",
      "mean_delta_AIC_BMM_minus_true",
      "mean_delta_AIC_BM1_minus_true",
      "mean_weight_true",
      "mean_weight_no_EB",
      "mean_weight_no_OUM",
      "mean_weight_BMM",
      "mean_weight_BM1",
      "true_wins",
      "true_wins_delta_gt_2_vs_no_EB",
      "true_wins_delta_gt_2_vs_no_OUM",
      "true_wins_delta_gt_10_vs_BM1"
    ),
    value = c(
      mean(results$AIC_true),
      mean(results$AIC_no_EB),
      mean(results$AIC_no_OUM),
      mean(results$AIC_BMM),
      mean(results$AIC_BM1),
      mean(results$AIC_no_EB - results$AIC_true),
      mean(results$AIC_no_OUM - results$AIC_true),
      mean(results$AIC_BMM - results$AIC_true),
      mean(results$AIC_BM1 - results$AIC_true),
      mean(weights[, "weight_true"]),
      mean(weights[, "weight_no_EB"]),
      mean(weights[, "weight_no_OUM"]),
      mean(weights[, "weight_BMM"]),
      mean(weights[, "weight_BM1"]),
      sum(wins == "AIC_true"),
      sum((results$AIC_no_EB - results$AIC_true) > 2),
      sum((results$AIC_no_OUM - results$AIC_true) > 2),
      sum((results$AIC_BM1 - results$AIC_true) > 10)
    )
  )
}
...
tree_seed <- env_int("MVSIMMAP_OUM_EB_BENCH_TREE_SEED", 20260330L)
n_tips <- env_int("MVSIMMAP_OUM_EB_BENCH_NTIPS", 100L)
nsim <- env_int("MVSIMMAP_OUM_EB_BENCH_REPS", 4L)
method <- env_chr("MVSIMMAP_OUM_EB_BENCH_METHOD", "rpf")
alpha_decomp <- env_chr("MVSIMMAP_OUM_EB_BENCH_ALPHA_DECOMP", "scalarPositive")
output_dir <- env_chr("MVSIMMAP_OUM_EB_BENCH_OUTPUT_DIR", "")
ncores <- env_int("MVSIMMAP_OUM_EB_BENCH_CORES", 4L)

if (!method %in% c("rpf", "inverse", "pseudoinverse")) {
  stop("MVSIMMAP_OUM_EB_BENCH_METHOD must be one of rpf, inverse, pseudoinverse", call. = FALSE)
}
if (!alpha_decomp %in% c("diagonalPositive", "scalarPositive")) {
  stop("MVSIMMAP_OUM_EB_BENCH_ALPHA_DECOMP must be one of diagonalPositive or scalarPositive", call. = FALSE)
}
...
seeds <- seq.int(4001L, length.out = nsim)
ncores <- min(ncores, parallel::detectCores(logical = FALSE), nsim)

scaffold <- mvSIMMAP(
  benchmark$tree,
  data = NULL,
  process = c(A = "BM", B = "EB", C = "OUM"),
  optimization = "fixed",
  method = method,
  param = list(
    ntraits = 3,
...
run_one <- function(i) {
  Y <- Ylist[[i]]
  fit_true <- fit_mixed_model(
    benchmark$tree, Y,
    process = c(A = "BM", B = "EB", C = "OUM"),
    method = method,
    decomp = alpha_decomp
  )
  fit_no_eb <- fit_mixed_model(
    benchmark$tree, Y,
    process = c(A = "BM", B = "BM", C = "OUM"),
    method = method,
    decomp = alpha_decomp
  )
  fit_no_oum <- fit_mixed_model(
    benchmark$tree, Y,
    process = c(A = "BM", B = "EB", C = "BM"),
    method = method,
    decomp = alpha_decomp
  )
  ...
  out <- flatten_true_fit(fit_true, rep_id = i, seed = seeds[i])

  out$AIC_no_EB <- fit_no_eb$AIC
  out$AICc_no_EB <- fit_no_eb$AICc
  out$logLik_no_EB <- as.numeric(logLik(fit_no_eb))
  out$nparam_no_EB <- fit_no_eb$param$nparam
  out$no_EB_convergence <- fit_no_eb$convergence
  out$no_EB_hessian <- fit_no_eb$diagnostics$hessian$status

  out$AIC_no_OUM <- fit_no_oum$AIC
  out$AICc_no_OUM <- fit_no_oum$AICc
  out$logLik_no_OUM <- as.numeric(logLik(fit_no_oum))
  out$nparam_no_OUM <- fit_no_oum$param$nparam
  out$no_OUM_convergence <- fit_no_oum$convergence
  out$no_OUM_hessian <- fit_no_oum$diagnostics$hessian$status
  ...
  out$delta_AIC_no_EB_minus_true <- fit_no_eb$AIC - fit_true$AIC
  out$delta_AIC_no_OUM_minus_true <- fit_no_oum$AIC - fit_true$AIC
  out$delta_AIC_BMM_minus_true <- fit_bmm$AIC - fit_true$AIC
  out$delta_AIC_BM1_minus_true <- fit_bm1$AIC - fit_true$AIC
  out
}
...
cat("\nTRUE_SIGMA_C_OUM\n")
print(true_param$sigma$C)

cat("\nTRUE_ALPHA_C_OUM\n")
print(true_param$alpha$C)

cat("\nTRUE_BETA_B_EB\n")
print(true_param$beta)
...
print(
  results[, c(
    ...
    "AIC_BMM",
    "AIC_BM1",
    "delta_AIC_no_EB_minus_true",
    "delta_AIC_no_OUM_minus_true",
    "delta_AIC_BMM_minus_true",
    "delta_AIC_BM1_minus_true",
    "true_convergence",
    "no_EB_convergence",
    "no_OUM_convergence",
    "true_hessian",
...
cat("\nTRUE_HESSIAN_TABLE\n")
print(table(results$true_hessian))
