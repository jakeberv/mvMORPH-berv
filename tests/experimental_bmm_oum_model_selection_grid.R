...
bootstrap_runtime <- function(repo_root) {
  ...
  install_lib <- Sys.getenv("CAL_GRID_R_LIB", unset = "")
  ...
  if (length(missing_pkgs)) {
    ...
    utils::install.packages(
      missing_pkgs,
      repos = Sys.getenv("CAL_GRID_CRAN_REPO", unset = "https://cloud.r-project.org"),
      lib = install_lib,
      quiet = TRUE
    )
  }
...
make_regime_counts <- function(n_tips, n_regimes, regime_balance) {
  ...
  if (identical(regime_balance, "imbalanced")) {
    ...
    weights <- rev(seq_len(n_regimes))
    return(normalize_counts(n_tips, weights))
  }
...
simulate_scalar_bm <- function(tree, sigma, mean_mat, seed) {
  Vphy <- ape::vcv.phylo(tree)
  simulate_gaussian_process(Vphy = Vphy, sigma = sigma, mean_mat = mean_mat, seed = seed)
}
...
simulate_truth <- function(truth, tree, alpha, base_sigma, theta_sep, rate_ratio, X_full, beta, seed) {
  ...
  if (truth == "shared_BM") {
    Y <- simulate_scalar_bm(tree = tree, sigma = base_sigma, mean_mat = zero_mean, seed = seed)
  } else if (truth == "shared_OU") {
    Y <- simulate_scalar_ou(tree = tree, alpha = alpha, sigma = base_sigma, mean_mat = zero_mean, seed = seed)
  } else if (truth == "pure_BMM") {
...
choose_best_model <- function(values, tol = 1e-8) {
  ...
  list(
    winner = if (length(winners) == 1L) winners[[1]] else "tie",
    best = best
  )
}

expected_overall_model <- function(truth) {
  switch(
    truth,
    shared_BM = "BM",
    shared_OU = "OU",
...
expected_family_winner <- function(truth) {
  switch(
    truth,
    shared_BM = "BM_family",
    pure_BMM = "BM_family",
...
expected_regime_detection <- function(truth) {
  switch(
    truth,
    shared_BM = "shared",
    shared_OU = "shared",
...
expected_regime_type <- function(truth) {
  switch(
    truth,
    pure_BMM = "BMM",
    pure_OUM = "OUM",
...
is_correct_choice <- function(observed, expected) {
  if (is.na(expected) || is.na(observed) || identical(observed, "tie")) return(NA)
...
write_checkpoint <- function(df, checkpoint_dir, scenario_id, rep_id) {
  ...
  out_path <- file.path(
    checkpoint_dir,
    sprintf("result_%s_rep%03d.csv", scenario_id, as.integer(rep_id))
  )
...
summarize_results <- function(results) {
  summary_table <- do.call(
    ...
    lapply(split(results, list(results$scenario_id, results$method), drop = TRUE), function(df) {
      data.frame(
        ...
        reps = nrow(df),
        bm_fit_rate = safe_rate(df$bm_fit_success),
        ou_fit_rate = safe_rate(df$ou_fit_success),
        bmm_fit_rate = safe_rate(df$bmm_fit_success),
        oum_fit_rate = safe_rate(df$oum_fit_success),
        all_gic_rate = safe_rate(df$all_gic_available),
        select_bm_rate = safe_rate(df$overall_winner == "BM"),
        select_ou_rate = safe_rate(df$overall_winner == "OU"),
        select_bmm_rate = safe_rate(df$overall_winner == "BMM"),
        select_oum_rate = safe_rate(df$overall_winner == "OUM"),
        tie_rate = safe_rate(df$overall_winner == "tie"),
        select_shared_rate = safe_rate(df$regime_detection_winner == "shared"),
        select_regime_rate = safe_rate(df$regime_detection_winner == "regime"),
        select_bm_family_rate = safe_rate(df$family_winner == "BM_family"),
        select_ou_family_rate = safe_rate(df$family_winner == "OU_family"),
        correct_overall_rate = safe_rate(df$correct_overall),
        correct_family_rate = safe_rate(df$correct_family),
        correct_regime_detection_rate = safe_rate(df$correct_regime_detection),
        correct_regime_type_rate = safe_rate(df$correct_regime_type),
        mean_delta_gic_bm_family_minus_ou_family = safe_mean(df$delta_gic_bm_family_minus_ou_family),
        sd_delta_gic_bm_family_minus_ou_family = safe_sd(df$delta_gic_bm_family_minus_ou_family),
        mean_delta_gic_regime_minus_shared = safe_mean(df$delta_gic_regime_minus_shared),
        sd_delta_gic_regime_minus_shared = safe_sd(df$delta_gic_regime_minus_shared),
        mean_delta_gic_bmm_minus_oum = safe_mean(df$delta_gic_bmm_minus_oum),
        sd_delta_gic_bmm_minus_oum = safe_sd(df$delta_gic_bmm_minus_oum),
        beta_rmse_bm_mean = safe_mean(df$beta_rmse_bm),
        beta_rmse_ou_mean = safe_mean(df$beta_rmse_ou),
        beta_rmse_bmm_mean = safe_mean(df$beta_rmse_bmm),
  ...
  bmm_frontier <- summary_table[summary_table$phase == "bmm_frontier", , drop = FALSE]
  if (nrow(bmm_frontier)) {
    boundary_rows[["bmm_frontier"]] <- do.call(
      rbind,
      lapply(split(
        bmm_frontier,
        list(
          ...
          bmm_frontier$n_tips,
          bmm_frontier$p,
          bmm_frontier$n_regimes,
          bmm_frontier$regime_balance,
          bmm_frontier$predictor,
          bmm_frontier$covariate_effect
        ),
        drop = TRUE
      ), function(df) {
        df <- df[order(df$rate_ratio), , drop = FALSE]
        signal <- df$rate_ratio
        select_rate <- df$select_bmm_rate
        ...
        data.frame(
          ...
          covariate_effect = df$covariate_effect[[1]],
          alpha = NA_real_,
          signal_parameter = "rate_ratio",
          select_rate_at_null_signal = select_rate[match(1, signal)],
  ...
  oum_frontier <- summary_table[summary_table$phase == "oum_frontier", , drop = FALSE]
  if (nrow(oum_frontier)) {
    boundary_rows[["oum_frontier"]] <- do.call(
      rbind,
      lapply(split(
        oum_frontier,
        list(
          ...
          oum_frontier$n_tips,
          oum_frontier$p,
          oum_frontier$n_regimes,
          oum_frontier$regime_balance,
          oum_frontier$predictor,
          oum_frontier$covariate_effect,
          oum_frontier$alpha
        ),
        drop = TRUE
      ), function(df) {
  ...
  issue_rows <- results[results$has_issue | !results$bm_fit_success | !results$ou_fit_success | !results$bmm_fit_success | !results$oum_fit_success, , drop = FALSE]
  ...
  family_summary <- family_summary[order(
    family_summary$phase,
    family_summary$truth,
    family_summary$predictor,
    family_summary$method,
    family_summary$regime_balance
  ), ]
...
cores <- env_int("CAL_GRID_CORES", max(1L, parallel::detectCores(logical = FALSE) %||% 1L))
seed_base <- env_int("CAL_GRID_SEED", 20260322L)
checkpoint_dir <- Sys.getenv("CAL_GRID_CHECKPOINT_DIR", unset = "")
phase_filter <- env_chr_subset(
  "CAL_GRID_PHASES",
  c("null_calibration", "bmm_frontier", "oum_frontier", "mixed_surface")
)
methods <- env_chr_subset("CAL_GRID_METHODS", c("LL"))
if (!all(methods %in% "LL")) {
  stop("This calibration study currently supports method='LL' only.", call. = FALSE)
}

regime_balances <- env_chr_subset("CAL_GRID_BALANCE", c("balanced", "imbalanced"))
...
frontier_predictors <- env_chr_subset(
  "CAL_GRID_FRONTIER_PREDICTOR",
  c("cont_confounded", "disc3_sparse_confounded")
)
mixed_predictors <- env_chr_subset(
  "CAL_GRID_MIXED_PREDICTOR",
  c("cont_confounded", "disc3_sparse_confounded")
)

covariate_levels <- env_num_subset("CAL_GRID_COVARIATE_EFFECT", c(0.00, 0.05))
regime_levels <- env_num_subset("CAL_GRID_REGIMES", c(2L, 3L))

null_reps <- env_int("CAL_GRID_NULL_REPS", 12L)
bmm_reps <- env_int("CAL_GRID_BMM_REPS", 8L)
oum_reps <- env_int("CAL_GRID_OUM_REPS", 6L)
mixed_reps <- env_int("CAL_GRID_MIXED_REPS", 6L)

null_bm <- expand.grid(
  phase = "null_calibration",
  truth = "shared_BM",
  n_tips = env_num_subset("CAL_GRID_NULL_NTIPS", c(60L, 120L)),
  p = env_num_subset("CAL_GRID_NULL_P", c(8L, 24L)),
...
null_ou <- expand.grid(
  ...
  predictor = null_predictors,
  covariate_effect = covariate_levels,
  alpha = env_num_subset("CAL_GRID_NULL_OU_ALPHA", c(0.25, 0.5, 1.5)),
  theta_sep = 0,
  rate_ratio = 1,
  reps = null_reps,
  stringsAsFactors = FALSE
...
bmm_frontier <- expand.grid(
  phase = "bmm_frontier",
  truth = "pure_BMM",
  n_tips = env_num_subset("CAL_GRID_BMM_NTIPS", c(60L, 120L)),
  p = env_num_subset("CAL_GRID_BMM_P", c(8L, 24L)),
  n_regimes = regime_levels,
  regime_balance = regime_balances,
  ...
  alpha = NA_real_,
  theta_sep = 0,
  rate_ratio = env_num_subset("CAL_GRID_BMM_RATE", c(1.00, 1.02, 1.05, 1.10, 1.20, 1.35, 1.50, 2.00)),
  reps = bmm_reps,
  stringsAsFactors = FALSE
)

oum_frontier <- expand.grid(
  phase = "oum_frontier",
  truth = "pure_OUM",
  n_tips = env_num_subset("CAL_GRID_OUM_NTIPS", c(60L, 120L)),
  p = env_num_subset("CAL_GRID_OUM_P", c(24L)),
  n_regimes = regime_levels,
  regime_balance = regime_balances,
  predictor = frontier_predictors,
  covariate_effect = covariate_levels,
  alpha = env_num_subset("CAL_GRID_OUM_ALPHA", c(0.10, 0.25, 0.50, 0.75, 1.00, 1.50)),
  theta_sep = env_num_subset("CAL_GRID_OUM_THETA", c(0.0000, 0.0025, 0.0050, 0.0100, 0.0200, 0.0400)),
  rate_ratio = 1,
  reps = oum_reps,
  stringsAsFactors = FALSE
)
...
mixed_grid <- expand.grid(
  phase = "mixed_surface",
  truth = "mixed",
  n_tips = env_num_subset("CAL_GRID_MIXED_NTIPS", c(120L)),
  p = env_num_subset("CAL_GRID_MIXED_P", c(24L)),
  n_regimes = regime_levels,
  regime_balance = regime_balances,
  predictor = mixed_predictors,
  covariate_effect = covariate_levels,
  alpha = env_num_subset("CAL_GRID_MIXED_ALPHA", c(0.25, 0.50, 1.00, 1.50)),
  theta_sep = env_num_subset("CAL_GRID_MIXED_THETA", c(0.0000, 0.0050, 0.0100, 0.0200, 0.0400)),
  rate_ratio = env_num_subset("CAL_GRID_MIXED_RATE", c(1.00, 1.05, 1.10, 1.20, 1.35, 1.50, 2.00)),
  reps = mixed_reps,
  stringsAsFactors = FALSE
)

scenario_grid <- rbind(
  null_bm[null_bm$phase %in% phase_filter, , drop = FALSE],
  null_ou[null_ou$phase %in% phase_filter, , drop = FALSE],
  bmm_frontier[bmm_frontier$phase %in% phase_filter, , drop = FALSE],
  oum_frontier[oum_frontier$phase %in% phase_filter, , drop = FALSE],
  mixed_grid[mixed_grid$phase %in% phase_filter, , drop = FALSE]
)
...
scenario_grid$scenario_id <- sprintf(
  ...
  ifelse(is.na(scenario_grid$alpha), "NA", format_level_id(scenario_grid$alpha)),
...
tasks <- do.call(
  ...
  lapply(seq_len(nrow(scenario_grid)), function(i) {
    data.frame(
      scenario_index = i,
      rep = seq_len(scenario_grid$reps[[i]]),
      stringsAsFactors = FALSE
    )
  })
)
...
run_replicate <- function(scenario, rep_id, scenario_index) {
  ...
  alpha_value <- if (is.na(scenario$alpha)) 0.5 else scenario$alpha
  Y <- simulate_truth(
  ...
  candidate_models <- c("BM", "OU", "BMM", "OUM")

  rows <- lapply(methods, function(method) {
    fit_results <- lapply(candidate_models, function(model) {
      fit_candidate(predictor$formula, data_full, tree, model, method)
    })
    names(fit_results) <- candidate_models

    gic_results <- lapply(candidate_models, function(model) {
      fit_obj <- fit_results[[model]]$fit
      if (inherits(fit_obj, "error")) {
        return(list(value = fit_obj, warnings = character()))
      }
      compute_gic_safe(fit_obj)
    })
    names(gic_results) <- candidate_models

    gic_values <- vapply(candidate_models, function(model) {
      value <- gic_results[[model]]$value
      if (inherits(value, "error")) return(NA_real_)
      as.numeric(value$GIC)
    }, numeric(1))
    names(gic_values) <- candidate_models

    overall_choice <- choose_best_model(gic_values)
    shared_choice <- choose_best_model(gic_values[c("BM", "OU")])
    regime_choice <- choose_best_model(gic_values[c("BMM", "OUM")])
    bm_family_choice <- choose_best_model(gic_values[c("BM", "BMM")])
    ou_family_choice <- choose_best_model(gic_values[c("OU", "OUM")])
    family_choice <- choose_best_model(c(BM_family = bm_family_choice$best, OU_family = ou_family_choice$best))
    regime_detection_choice <- choose_best_model(c(shared = shared_choice$best, regime = regime_choice$best))

    warnings_by_model <- lapply(candidate_models, function(model) {
      collapse_messages(c(
        fit_results[[model]]$warnings,
        gic_results[[model]]$warnings,
        if (inherits(fit_results[[model]]$fit, "error")) conditionMessage(fit_results[[model]]$fit) else character()
      ))
    })
    names(warnings_by_model) <- candidate_models

    expected_overall <- expected_overall_model(scenario$truth)
    expected_family <- expected_family_winner(scenario$truth)
    expected_detection <- expected_regime_detection(scenario$truth)
    expected_regime <- expected_regime_type(scenario$truth)

    best_bm_family <- bm_family_choice$best
    best_ou_family <- ou_family_choice$best
    best_shared <- shared_choice$best
    best_regime <- regime_choice$best
    ...
    data.frame(
      ...
      rep = rep_id,
      method = method,
      bm_fit_success = !inherits(fit_results$BM$fit, "error"),
      ou_fit_success = !inherits(fit_results$OU$fit, "error"),
      bmm_fit_success = !inherits(fit_results$BMM$fit, "error"),
      oum_fit_success = !inherits(fit_results$OUM$fit, "error"),
      all_gic_available = all(is.finite(gic_values)),
      gic_bm = gic_values[["BM"]],
      gic_ou = gic_values[["OU"]],
      gic_bmm = gic_values[["BMM"]],
      gic_oum = gic_values[["OUM"]],
      overall_winner = overall_choice$winner,
      shared_winner = shared_choice$winner,
      regime_winner = regime_choice$winner,
      family_winner = family_choice$winner,
      regime_detection_winner = regime_detection_choice$winner,
      correct_overall = is_correct_choice(overall_choice$winner, expected_overall),
      correct_family = is_correct_choice(family_choice$winner, expected_family),
      correct_regime_detection = is_correct_choice(regime_detection_choice$winner, expected_detection),
      correct_regime_type = is_correct_choice(regime_choice$winner, expected_regime),
      delta_gic_bm_family_minus_ou_family = if (is.finite(best_bm_family) && is.finite(best_ou_family)) best_ou_family - best_bm_family else NA_real_,
      delta_gic_regime_minus_shared = if (is.finite(best_shared) && is.finite(best_regime)) best_shared - best_regime else NA_real_,
      delta_gic_bmm_minus_oum = if (is.finite(gic_values[["BMM"]]) && is.finite(gic_values[["OUM"]])) gic_values[["OUM"]] - gic_values[["BMM"]] else NA_real_,
      beta_rmse_bm = extract_beta_rmse(fit_results$BM$fit, beta_true),
      beta_rmse_ou = extract_beta_rmse(fit_results$OU$fit, beta_true),
      beta_rmse_bmm = extract_beta_rmse(fit_results$BMM$fit, beta_true),
      beta_rmse_oum = extract_beta_rmse(fit_results$OUM$fit, beta_true),
      alpha_hat_ou = extract_alpha_hat(fit_results$OU$fit),
      alpha_hat_oum = extract_alpha_hat(fit_results$OUM$fit),
      bm_messages = warnings_by_model[["BM"]],
      ou_messages = warnings_by_model[["OU"]],
      bmm_messages = warnings_by_model[["BMM"]],
...
cat(
  "Running four-model calibration grid with",
...
family_csv <- Sys.getenv("CAL_GRID_FAMILY_OUT", unset = "")
if (nzchar(family_csv)) {
  utils::write.csv(family_summary, family_csv, row.names = FALSE)
  cat("Saved family and regime summary to", family_csv, "\n")
}

boundary_csv <- Sys.getenv("CAL_GRID_BOUNDARY_OUT", unset = "")
...
issue_csv <- Sys.getenv("CAL_GRID_ISSUE_OUT", unset = "")
if (nzchar(issue_csv)) {
