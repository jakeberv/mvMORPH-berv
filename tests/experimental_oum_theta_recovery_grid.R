...
bootstrap_runtime <- function(repo_root) {
  ...
  install_lib <- Sys.getenv("OUM_REC_R_LIB", unset = "")
  ...
  if (length(missing_pkgs)) {
    ...
    utils::install.packages(
      missing_pkgs,
      repos = Sys.getenv("OUM_REC_CRAN_REPO", unset = "https://cloud.r-project.org"),
      lib = install_lib,
      quiet = TRUE
    )
  }
...
simulate_truth <- function(truth, tree, alpha, base_sigma, theta_sep, X_full, beta, seed) {
  p <- ncol(base_sigma)
  ...
  list(
    Y = Y,
    theta = theta,
    theta_spread = theta_spread(theta)
  )
}
...
extract_theta_metrics <- function(fit_object, theta_true) {
  spread_true <- theta_spread(theta_true)
  default <- list(
    theta_rmse = NA_real_,
    theta_bias = NA_real_,
    theta_cor = NA_real_,
    theta_spread_hat = NA_real_,
    theta_spread_abs_error = NA_real_,
    theta_spread_true = spread_true
  )
  if (inherits(fit_object, "error")) return(default)

  coef_est <- coef(fit_object)
  if (!all(rownames(theta_true) %in% rownames(coef_est))) return(default)
  theta_est <- coef_est[rownames(theta_true), , drop = FALSE]
  if (ncol(theta_est) != ncol(theta_true)) return(default)
  if (!is.null(colnames(theta_true))) {
    colnames(theta_est) <- colnames(theta_true)
  }

  theta_est <- as.matrix(theta_est)
  theta_true <- as.matrix(theta_true)
  flat_est <- as.numeric(theta_est)
  flat_true <- as.numeric(theta_true)
  theta_cor <- if (
    stats::sd(flat_est) > 0 &&
    stats::sd(flat_true) > 0
  ) {
    stats::cor(flat_est, flat_true)
  } else {
    NA_real_
  }

  spread_hat <- theta_spread(theta_est)

  list(
    theta_rmse = sqrt(mean((theta_est - theta_true)^2)),
    theta_bias = mean(theta_est - theta_true),
    theta_cor = theta_cor,
    theta_spread_hat = spread_hat,
    theta_spread_abs_error = abs(spread_hat - spread_true),
    theta_spread_true = spread_true
  )
}
...
summarize_results <- function(results) {
  summary_table <- do.call(
    ...
    lapply(split(results, results$scenario_id, drop = TRUE), function(df) {
      data.frame(
        ...
        theta_spread_true = df$theta_spread_true[[1]],
        theta_rmse_oum_mean = safe_mean(df$theta_rmse_oum),
        theta_rmse_oum_selected_mean = safe_mean(df$theta_rmse_oum[df$winner == "OUM"]),
        theta_bias_oum_mean = safe_mean(df$theta_bias_oum),
        theta_cor_oum_mean = safe_mean(df$theta_cor_oum),
        theta_cor_oum_selected_mean = safe_mean(df$theta_cor_oum[df$winner == "OUM"]),
        theta_spread_hat_oum_mean = safe_mean(df$theta_spread_hat_oum),
        theta_spread_bias_oum_mean = safe_mean(df$theta_spread_hat_oum - df$theta_spread_true),
        theta_spread_abs_error_oum_mean = safe_mean(df$theta_spread_abs_error_oum),
  ...
  recovery_phase <- summary_table[summary_table$phase == "theta_recovery", , drop = FALSE]
  boundary_summary <- if (nrow(recovery_phase)) {
    do.call(
      rbind,
      lapply(split(
        recovery_phase,
        list(
          recovery_phase$method,
          recovery_phase$n_tips,
          recovery_phase$p,
          recovery_phase$predictor,
          recovery_phase$regime_balance,
          recovery_phase$alpha
        ),
        drop = TRUE
      ), function(df) {
        ...
        data.frame(
          ...
          select_rate_at_min_signal = select_rate[[1]],
...
cores <- env_int("OUM_REC_CORES", max(1L, parallel::detectCores(logical = FALSE) %||% 1L))
seed_base <- env_int("OUM_REC_SEED", 20260323L)
checkpoint_dir <- Sys.getenv("OUM_REC_CHECKPOINT_DIR", unset = "")
phase_filter <- env_chr_subset("OUM_REC_PHASES", c("focused_null", "theta_recovery"))
methods <- env_chr_subset("OUM_REC_METHODS", c("LL"))
if (!all(methods %in% "LL")) {
  stop("This theta recovery study currently supports method='LL' only.", call. = FALSE)
}

regime_levels <- as.integer(env_num_subset("OUM_REC_REGIMES", c(2L)))
regime_balances <- env_chr_subset("OUM_REC_BALANCE", c("imbalanced"))
null_predictors <- env_chr_subset(
  "OUM_REC_NULL_PREDICTOR",
  c("cont_indep", "cont_confounded", "disc_binary", "disc3_sparse_confounded")
)
recovery_predictors <- env_chr_subset(
  "OUM_REC_RECOVERY_PREDICTOR",
  c("cont_indep", "cont_confounded", "disc_binary", "disc3_sparse_confounded")
)
covariate_levels <- env_num_subset("OUM_REC_COVARIATE_EFFECT", c(0.00, 0.05))

null_grid <- expand.grid(
  phase = "focused_null",
  truth = "shared_OU",
  n_tips = env_num_subset("OUM_REC_NULL_NTIPS", c(60L, 120L, 240L)),
  p = env_num_subset("OUM_REC_NULL_P", c(8L, 24L, 48L)),
  n_regimes = regime_levels,
  regime_balance = regime_balances,
  predictor = null_predictors,
  covariate_effect = covariate_levels,
  alpha = env_num_subset("OUM_REC_NULL_ALPHA", c(0.50, 1.00, 1.50)),
  theta_sep = 0,
  reps = env_int("OUM_REC_NULL_REPS", 12L),
  stringsAsFactors = FALSE
)

recovery_grid <- expand.grid(
  phase = "theta_recovery",
  truth = "pure_OUM",
  n_tips = env_num_subset("OUM_REC_RECOVERY_NTIPS", c(60L, 120L, 240L)),
  p = env_num_subset("OUM_REC_RECOVERY_P", c(8L, 24L, 48L)),
  n_regimes = regime_levels,
  regime_balance = regime_balances,
  predictor = recovery_predictors,
  covariate_effect = covariate_levels,
  alpha = env_num_subset("OUM_REC_RECOVERY_ALPHA", c(0.50, 1.00, 1.50)),
  theta_sep = env_num_subset("OUM_REC_RECOVERY_THETA", c(0.0800, 0.1200, 0.1800, 0.2400, 0.3000, 0.3600, 0.4500, 0.6000)),
  reps = env_int("OUM_REC_RECOVERY_REPS", 10L),
  stringsAsFactors = FALSE
)

scenario_grid <- rbind(
  null_grid[null_grid$phase %in% phase_filter, , drop = FALSE],
  recovery_grid[recovery_grid$phase %in% phase_filter, , drop = FALSE]
)

balance_id <- ifelse(scenario_grid$regime_balance == "balanced", "bal", "imb")
scenario_grid$scenario_id <- sprintf(
  "%s_%s_n%d_p%d_r%d_%s_%s_c%s_a%s_t%s",
  scenario_grid$phase,
  scenario_grid$truth,
...
run_replicate <- function(scenario, rep_id, scenario_index) {
  ...
  rows <- lapply(methods, function(method) {
    ...
    theta_metrics <- extract_theta_metrics(fit_oum$fit, truth$theta)
    ...
    data.frame(
      ...
      theta_spread_true = theta_metrics$theta_spread_true,
      theta_rmse_oum = theta_metrics$theta_rmse,
      theta_bias_oum = theta_metrics$theta_bias,
      theta_cor_oum = theta_metrics$theta_cor,
      theta_spread_hat_oum = theta_metrics$theta_spread_hat,
      theta_spread_abs_error_oum = theta_metrics$theta_spread_abs_error,
...
cat(
  "Running focused OUM theta recovery grid with",
...
cat("\nCompleted focused OUM theta recovery grid.\n")
...
boundary_csv <- Sys.getenv("OUM_REC_BOUNDARY_OUT", unset = "")
...
issue_csv <- Sys.getenv("OUM_REC_ISSUE_OUT", unset = "")
if (nzchar(issue_csv)) {
