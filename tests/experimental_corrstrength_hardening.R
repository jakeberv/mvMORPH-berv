...
profile_helper <- NULL
for (nm in c(".mvgls_bmm_corrstrength_profile", ".mvgls_corrstrength_profile",
...
fit_corrstrength <- function(formula, data, tree, start = NULL, bmm.reference = NULL) {
  mvgls(
    formula,
    data = data,
    tree = tree,
...
get_diag <- function(fit) {
  ...
  if (is.null(diag)) stop("corr-strength fit did not populate diagnostics$corrstrength", call. = FALSE)
  diag
}
...
check_start_table <- function(fit, expected_nstarts, expected_source = NULL) {
  diag <- get_diag(fit)
  st <- diag$start_table
  assert_true(is.data.frame(st), "diagnostics$corrstrength$start_table must be a data.frame")
  assert_true(nrow(st) == expected_nstarts, sprintf("expected %d starts, got %d", expected_nstarts, nrow(st)))
  required <- c("start_id", "source", "scale_multiplier", "corr_strength_seed", "convergence", "nloglik", "max_scale", "selected")
...
easy_fit <- fit_corrstrength(Y ~ 1, data = list(Y = easy_Y), tree = easy_tree)
...
assert_true(any(grepl("corr_strength", summary_out, fixed = TRUE)), "summary output did not report corr-strength terminology")
...
alt_ref_fit_name <- fit_corrstrength(Y ~ 1, data = list(Y = easy_Y), tree = easy_tree, bmm.reference = "B")
alt_ref_fit_index <- fit_corrstrength(Y ~ 1, data = list(Y = easy_Y), tree = easy_tree, bmm.reference = 2L)
...
assert_true(is.finite(unname(alt_ref_fit_name$param["B.corr_strength"])),
  "the selected reference regime must retain a finite corr_strength estimate"
)
...
user_fit <- fit_corrstrength(Y ~ 1, data = list(Y = easy_Y), tree = easy_tree, start = user_start)
...
hard_fit <- fit_corrstrength(Y ~ 1, data = list(Y = hard_Y), tree = hard_tree)
hard_diag <- get_diag(hard_fit)
assert_true(
  isTRUE(hard_diag$boundary_corr_strength) || isTRUE(hard_diag$pathological_scale),
  "difficult seeded case did not trigger boundary/pathology diagnostics"
)

profile_corr_strength_max <- easy_diag$corr_strength_max
if (!is.finite(profile_corr_strength_max) || profile_corr_strength_max <= 0.06) {
  profile_corr_strength_max <- 1.5
}
profile_grid_corr_strength <- seq(0.05, min(1.5, 0.95 * profile_corr_strength_max), length.out = 5L)
profile_grid_scale <- c(0.5, 1, 2, 4)

corr_strength_profile <- tryCatch(
  do.call(profile_helper, list(
    object = easy_fit,
    parameter = "corr_strength",
    regime = "B",
    grid = profile_grid_corr_strength
  )),
  error = function(e) e
)
if (inherits(corr_strength_profile, "error")) stop(conditionMessage(corr_strength_profile), call. = FALSE)
assert_true(is.data.frame(corr_strength_profile), "corr_strength profile helper did not return a data.frame")
assert_true(all(c("fixed_value", "logLik", "convergence", "max_scale") %in% names(corr_strength_profile)),
  "corr_strength profile helper returned unexpected columns"
)
assert_true(nrow(corr_strength_profile) == length(profile_grid_corr_strength), "corr_strength profile helper returned the wrong number of rows")
assert_true(sum(is.finite(corr_strength_profile$logLik)) >= 4L, "corr_strength profile helper should return mostly finite log-likelihoods")
reference_corr_strength_profile <- tryCatch(
  do.call(profile_helper, list(
...
if (inherits(reference_corr_strength_profile, "error")) {
  stop(conditionMessage(reference_corr_strength_profile), call. = FALSE)
}
assert_true(is.data.frame(reference_corr_strength_profile),
  "reference corr_strength profile should return a data.frame"
)
